  ...
  useEffect(() => {
    fetch('http://localhost:3000/movies')
      ...
      .then(data => {
        ...
        const format = data.movies.map(movie => {
        const poster = `https://image.tmdb.org/t/p/w500/${movie.poster_path}`;
        return { title: movie.title, poster : poster, voteAverage: movie.vote_average, voteCount: movie.vote_count, overview: movie.overview.substr(0,250)+"..." };
      
      });
         setMoviesData(format);
  });
 }, []);
